...
class DistributorsController extends Controller
{
    ...
    public function index()
    {
      ...
      {
                $data = \App\Distributors::join('users','users.id', '=', 'slj_distributors.user_id')
                ->leftJoin('slj_cities','slj_distributors.city', '=', 'slj_cities.id')
                ->select('slj_distributors.*','slj_cities.name as city_name','users.mobile','users.email','users.status')
      ...
      else
      {
       ...
       \App\Employees_Logs::create($employeedata);            
		...
		return view('administration.distributors.index',['data'=>$data]);
 ...
 $itemname = \App\StoreItems::select('slj_stock_items.*')->where('id',$id)->first();
    ...
    public function store(Request $request)
    {
        ...
        //Assign Role
        $userdata->assignRole('distributor');
        
        $employeedata=array();
	    $id = \Auth::user()->id;
      
       $employeedata['employee_id']=$id;
       $employeedata['action_name']=$input['name']."Distributor Was Created";
       \App\Employees_Logs::create($employeedata);
	

        //Generate Franchise Id
        $data = \App\Distributors::select('distributor_id')->orderBy('distributor_id','desc')->first();
        $distributorid = "00000";
    ...
    {
        return view('administration.distributors.show');
    }
    ...
    public function update(Request $request, $id)
    {
       ...
       $employeedata['employee_id']=$id;
       $employeedata['action_name']=$requestdata['name']."Distributor Was Updated";
       \App\Employees_Logs::create($employeedata);
	
		
		return redirect('admin/distributors')->with('success', 'Distributor details updated successfully.');
    }
