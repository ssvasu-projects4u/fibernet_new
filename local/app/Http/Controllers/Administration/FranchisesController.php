    ...
    public function destroy($id)
    {
        ...
        if($id > 0 && $customers == 0 && $dpid==0){
          ...
          //  \App\Franchises::destroy($id);
			  $data['status'] = "N";
            $franchises = \App\Franchises::find($id);
            
            //Update details
            $franchises->update($data);
...
DB::enableQueryLog();
    ...
    //    echo 'hi'.$roles[0];die;
        ...
        if($ids){
            ...
            $distributors = \App\Branches::join('users','users.id', '=', 'slj_branches.user_id')->where('city',$city)->where('distributor_id',$distributor)->whereIn('slj_branches.id',$ids)->where('users.status','Y')->select('slj_branches.id','branch_name')->get();
        }
        $branchdetails=array();
        if($roles[0]=='superadmin')
        {
...
// dd(formatQuery($lastQuery));
        ...
        else
        {
            $branchdetails = \App\Employees::select('branch')->where('user_id',$id)->first();
            $branchgroup = explode(",",$branchdetails->branch);
            $distributors = \App\Branches::select('slj_branches.id','branch_name')->whereIn('slj_branches.id',$branchgroup)->get();
   		}
	 ...
	 public function getCitySubdistributorBranches($city,$subdistributor)
    {
        $id = \Auth::user()->id;
        $roles = \Auth::user()->getRoleNames(); 
        $ids=array();
        ...
        if($ids){
            ...
            $subdistributors = \App\Branches::join('users','users.id', '=', 'slj_branches.user_id')->where('subdistributor_id',$subdistributor)->whereIn('slj_branches.id',$ids)->where('users.status','Y')->select('slj_branches.id','branch_name')->get();
        }
        $branchdetails=array();
        if($roles[0]=='superadmin')
        {
               $subdistributors = \App\Branches::join('users','users.id', '=', 'slj_branches.user_id')->where('subdistributor_id',$subdistributor)->where('users.status','Y')->select('slj_branches.id','branch_name')->get();
        }
        else
        {
            $branchdetails = \App\Employees::select('branch')->where('user_id',$id)->first();
            $branchgroup = explode(",",$branchdetails->branch);
            $subdistributors = \App\Branches::select('slj_branches.id','branch_name')->whereIn('slj_branches.id',$branchgroup)->get();
   		}

        $html = "<option value=''>-- Select Branch --</option>";
        foreach($subdistributors as $subdistributor){
			$html.="<option value='".$subdistributor->id."'>".$subdistributor->subdistributors_name."</option>";
        }

        return $html;
    }
	
	 public function getSubDistributorsBranches($subdistributor)
    {
        //$user = Auth::user();
        $id = \Auth::user()->id;
        $roles = \Auth::user()->getRoleNames(); 
        ...
        if($ids){ 
            $branches = \App\Branches::join('users','users.id', '=', 'slj_branches.user_id')->where('subdistributor_id',$subdistributor)->whereIn('slj_branches.id',$ids)->where('users.status','Y')
        ->select('slj_branches.id','branch_name')->get();
        }
        else{ 
            $branches = \App\Branches::join('users','users.id', '=', 'slj_branches.user_id')->where('subdistributor_id',$subdistributor)->where('users.status','Y')
        ->select('slj_branches.id','branch_name')->get();
        }
        ...
        foreach($branches as $bran){
            $html.="<option value='".$bran->id."'>".$bran->branch_name."</option>";
        }
