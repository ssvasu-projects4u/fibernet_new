...
@if(Request::segment(2) == 'branches')
  @can("create-distributor")
    <li class="nav-item"><a class="nav-link utility-branchmove" rId="{{$id}}" href="javascript:void(0);">Create Utility</a></li>
  @endcan
@can("create-distributor")
...
@endif
 ...
 {!! Form::open(['route' =>['storebranchutilitiemoving'],'method'=>'post','files'=>'true'])!!}
	      @csrf
       ...
       <div class="row">
       <input type="submit" class="btn btn-success" id="submitbranchmoving"
name="submitbranchmoving" value="Submit">
         {!! Form::close() !!} 
...
<script>
		...
		$('#distributor').on('change', function() {
            ...
            $.ajax({
                url: "{{url('/admin/franchises/citydistributorbranchesutiilty')}}/"+city+"/"+distributor+"/"+branch,
                type: "GET",
                success:function(data) {
                   $('#branch').html(data);
                },
...
<script>
     $('.utility-branchmove').on('click', function () {
...
<script>
    $(document).ready(function() {
        ...
        $('#move_to').on('change', function() {
             ...
             //alert(branch);                                 
              $.ajax({
              url: "{{url('admin/distributors/getbranchoffices')}}/"+office+"/"+branch,
                  type: "GET",
                success:function(data) {
                   // alert(data);
                   $('#Offices').html(data);
