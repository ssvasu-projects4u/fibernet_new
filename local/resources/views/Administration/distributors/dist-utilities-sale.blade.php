...
@if(Request::segment(2) == 'distributors')
  @can("create-distributor")
    <li class="nav-item"><a class="nav-link utility-distributor3" rId="{{$id}}" href="javascript:void(0);">Create Utility</a></li>
  @endcan
  @can("create-distributor")
...
@endif
	...
	<div class="card-header py-2">
	  <div class="float-left"><h4 class="m-0 font-weight-bold text-primary">Sale Utility</h4></div>
 ...
 {!! Form::open(['route' =>['storeutilitysale'],'method'=>'post','files'=>'true'])!!}
	      @csrf
		...
		<div class="form-group col-md-3"> {!! Form::label('serial_no', 'Serial  Number') !!}
		 ...
		 {!! Form::text('buyingprice',null, array('class' => 'form-control','placeholder'=>'Enter Serial Number')) !!} 
		 </div>
		
		 <div class="form-group col-md-3"> {!! Form::label('Sale_Customer', 'Sale_Customer') !!}
		 {!! Form::text('Sale_Customer',null, array('class' => 'form-control','placeholder'=>'Enter Sale Customer Name')) !!} 
		 </div>
		 <div class="form-group col-md-3"> {!! Form::label('price', 'Sale Price') !!}
		 {!! Form::text('price',null, array('class' => 'form-control','placeholder'=>'Enter Product Price Value')) !!} 
		 </div>
       ...
       <div class="row">
       <input type="submit" class="btn btn-success" id="submitsale"
...
<script>
     $('.utility-distributor3').on('click', function () {
                                        var rid = $(this).attr('rId');
                                       $('#utilitydistributor').val(rid);
                                        $('#kingdist12').modal('show');
...
<script>
	...
	-->  
	 <div id="kingdist12" class="modal fade" role="dialog" aria-labelleddy="myModalLabel" style="display:none;">
    ...
    $(document).ready(function() {
        $('#itemname').on('change', function() {
          ...
          //  alert(item);
            ...
            $.ajax({
              url: "{{url('admin/distributors/getbuyprice')}}/"+item,
                  type: "GET",
                success:function(data) {
                  //  alert(data);
                   $('#buyingprice').val(data);
                },
                error: function(XMLHttpRequest, textStatus, errorThrown) {
                    alert(errorThrown);
                }
