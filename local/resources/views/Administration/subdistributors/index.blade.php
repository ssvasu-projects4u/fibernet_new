...
@section('content')
	...
	@foreach ($data as $datarow)
        ...
        <ul class="dropdown-menu dropdown-default" role="menu">
					@can("edit-distributor")
						<li><a class="dropdown-item" href="{{url('admin/subdistributors/edit/'.$datarow->id)}}"><i class="fa fa-pencil-alt" aria-hidden="true"></i> Edit</a></li>
          @endcan
          @can("delete-distributor")
						<li class="divider"><hr class="py-0 my-0 mb-4"></li>
						<li><a class="dropdown-item" href="{{url('admin/subdistributors/delete/'.$datarow->id)}}" onclick="return confirm('Are you sure to delete?')"><i class="fa fa-trash" aria-hidden="true"></i> Delete</a></li>
          @endcan
		...
		<td>@can("edit-distributor") <a href="{{url('admin/subdistributors/edit/'.$datarow->id)}}" title='edit'><i class="fa fa-pencil-alt" aria-hidden="true"></i> </a>@endcan {{  $datarow->subdistributor_name }}</td>
    ...
    <?php } else { ?>
        ...
        @endcanany
		 ...
		 @php $sdistdet = DB::table('slj_subdistributors')->where('id', $datarow)->first();
         $city = DB::table('slj_cities')->where('id', $sdistdet->city)->first();
         $userdet = DB::table('users')->select('email','mobile','status')->where('id', $sdistdet->user_id)->first();
		 $distdet = DB::table('slj_distributors')->select('distributor_name')->where('id', $sdistdet->distributor_id)->first();
		
		
		 @endphp
		<td>@can("edit-distributor") <a href="{{url('admin/subdistributors/edit/'.$datarow)}}" title='edit'><i class="fa fa-pencil-alt" aria-hidden="true"></i> </a>@endcan {{ $distdet->distributor_name }}</td>
		<!--<td>{{ $sdistdet->subdistributor_name }}</td>-->
		<td>{{ $city->name }}</td>
		<td>{{ $sdistdet->office_address }}</td>
