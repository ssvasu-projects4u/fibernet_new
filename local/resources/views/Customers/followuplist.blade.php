...
@section('content')
	...
	<div class="card-header py-2">
	  <div class="float-left"><h3 class="m-0 font-weight-bold text-primary">Follow-Up List</h3></div>
	...
	<div class="card-body" style="padding:5px;">
	  ...
	  <tr class="table-warning">
		...
		<th>Mobile</th>
		<th>Branch</th>
		<th>Internet Competetator Name</th>
	 <th>Cable Competator Name</th>
	 <th>Status</th>
	 <th>Followup(Next) Date & Time</th>
  ...
  @php
  $ss="Followup";
  $data1 = DB::table('prospect')->where('active','Y')->where('status',$ss)->OrderBy('status','desc')->get();
   $ss="Willing";
  $data1 = DB::table('prospect')->where('active','Y')->where('status',$ss)->OrderBy('status','desc')->get();
  @endphp
        ...
        <button class="btn btn-primary btn-sm btn-demo-space py-0" data-toggle="dropdown"><i class="fas fa-bars"></i></button>
          ...
          <ul class="dropdown-menu dropdown-default" role="menu">
            @can("delete-fh")
              ...
              <li><a class="dropdown-item" href="{{url('admin/customers/followup/intr/'.$datarow->id)}}" title='Intrested')"> Intrested</a> </li>
               @php $no=$datarow->id;@endphp
            @endcan
            @can("delete-fh")
              <li class="divider"><hr class="py-0 my-0 mb-4"></li>
              <li><a class="dropdown-item" href="{{url('admin/customers/followup/notintr/'.$datarow->id)}}" title='Intrested')"> Not-Intrested</a> </li>
               @php $no=$datarow->id;@endphp
            @endcan
            @can("delete-fh")
              <li class="divider"><hr class="py-0 my-0 mb-4"></li>
              <li><a class="dropdown-item" href="{{url('admin/customers/prospect/delete/'.$datarow->id)}}" title='delete' onclick="return confirm('Are you sure to delete?')"><i class="fa fa-trash" aria-hidden="true"></i> Delete</a> </li>
               @php $no=$datarow->id;@endphp
            @endcan
    ...
    <div class="modal-dialog" role="document">
		...
		<td>
		    ...
		    if($datarow->intercompany!='')
		    {
		    ?>
		    {{ $datarow->intercompany }}
		   <?php } else { echo "Not Using"; } ?>
		    
		    </td>
		<td>
		    <?php
		     if($datarow->cablecompany!='')
		    {
		    ?>
		       {{ $datarow->cablecompany }}
		    <?php } else { echo "Not Using"; } ?>
...
<div id="modalRegister" class="modal fade" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
         {!! Form::open(['route' =>['customers.updatefollowmdata'],'id'=>'submit7','method'=>'post'])!!}
	   	<div class="row">
	     ...
	     {!! Form::textarea('address',null, array('class' => 'form-control','rows'=>2,'required'=>'required','placeholder'=>'Enter Address')) !!} </div>
		   ...
		   <div class="form-group col-md-3"> {!! Form::label('Time', 'Time') !!}
	        {!! Form::time('caltime',null, array('class' => 'form-control')) !!} </div>
					    <div class="form-group col-md-3"> {!! Form::label('requiredservice', 'Required Service') !!}
            ...
            <input type="submit" value="Submit" name="submit7" id="submit7" class="btn btn-success">
