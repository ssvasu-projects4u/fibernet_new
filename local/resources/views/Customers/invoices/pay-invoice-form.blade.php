  ...
  {!! Form::model($data, ['method' => 'POST','route' => ['in.pay-invoice-store', $data->id]]) !!}
          @csrf
            <div class="row">
                ...
                ?>
                <div class="form-group col-md-3"> {!! Form::label('payment_type', 'Payment Type') !!}
                {!! Form::select('payment_type', $paymenttypes, null,array('class' => 'form-control','required'=>'required','placeholder'=>'select payment type') ) !!} @if ($errors->has('payment_type'))
                        <span class="help-block">
