...
@section('content')
	...
	<div class="card-body" style="padding:5px;">
	  @if(Session::has('success'))
		...
		<?php
			...
			//	echo '<script type="text/javascript">alert("'.$employee_search.'");</script>';
			}
	...
	<div class="form-group col-md-3"> {!! Form::label('employee', 'Employee') !!}
		...
		<div class="form-group col-md-3"><br> <a href="{{ url('admin/inventory/stocks/employee-wise') }}" class="btn btn-primary">Reset</a> </div>
		</div>
	...
	<script src="{{asset('assets/js/autocomplete/jquery-ui.min.js')}}"></script>
      ...
      $('#brand').val("");
        ...
        var brand = $('#brand').val();
        location.href = "{{ url('admin/inventory/stocks/employee-wise')}}?brand="+brand+"&category"+category+"&sub_category"+sub_category+"&employee="+ui.item.id+"&employee_search="+ui.item.value+"&city="+$('#city').val();
			},
			messages: {
				noResults: '',
				results: function() {}
     ...
     // alert(employee);
        ...
        location.href = "{{ url('admin/inventory/stocks/employee-wise')}}?brand="+brand+"&city="+city+"&employee="+employee+"&category="+category;
       	
      } else {
				location.href = "{{ url('admin/inventory/stocks/employee-wise')}}?brand="+brand+"&city="+city+"&employee="+employee;
				$('#employee_search').val(employee);
			}
		});
        ...
        location.href = "{{ url('admin/inventory/stocks/employee-wise')}}?brand="+brand+"&city="+city+"&employee={{$employee}}&category="+category+"&sub_category="+sub_category;
      }
      else {
  			location.href = "{{ url('admin/inventory/stocks/employee-wise')}}?brand="+brand+"&city="+city+"&employee={{$employee}}&category="+category;
			}
        ...
        location.href = "{{ url('admin/inventory/stocks/employee-wise')}}?employee={{$employee}}&city="+city+"&brand="+ui.item.value+"&category="+category+"&sub_category="+sub_category;
			},
			messages: {
				noResults: '',
				results: function() {}
