...
@section('content')
	...
	<div class="card-header py-2">
	  <div class="float-left"><h3 class="m-0 font-weight-bold text-primary">Branch Stocks</h3></div>
	...
	<div class="card-body" style="padding:5px;">
	  @if(Session::has('success'))
		...
		<div class="form-group col-md-4"> {!! Form::label('branch', 'Branch') !!}
		...
		<div class="form-group col-md-3"><br> <a href="{{ url('admin/inventory/stocks/fiberbranch-wise') }}" class="btn btn-primary">Reset</a> </div>
		</div>
	...
	@foreach ($data as $datarow)
        ...
        <td align="center"><div class="btn-group">
          ...
          @canany([
            "transfer-from-branch-to-franchise",
            "transfer-from-branch-to-employee"
          ])
            <ul class="dropdown-menu dropdown-default" role="menu">
				      @can("transfer-from-branch-to-franchise")
        	      <li><a class="dropdown-item" href="{{url('admin/inventory/stocks/transfer-to-franchise/branch-wise/'.$datarow->id.'?'.$querystring)}}"><i class="fa fa-reply" aria-hidden="true"></i> Transfer to Franchise</a></li>
				      @endcan
			  	    @can("transfer-from-branch-to-employee")
              	<li><a class="dropdown-item" href="{{url('admin/inventory/stocks/transfer-to-employee/branch-wise/'.$datarow->id.'?'.$querystring)}}"><i class="fa fa-reply" aria-hidden="true"></i> Transfer to Employee</a></li>
				      @endcan        
            </ul>
          @endcanany
            </div>
	...
	<script src="{{asset('assets/js/autocomplete/jquery-ui.min.js')}}"></script>
  	<script type="text/javascript">
		...
		$('#distributor').on('change', function() {
			...
			$.ajax({
				url: "{{url('inventory/get-branch-by')}}/"+city+"/"+distributor,
				type: "GET",
				success:function(data) {
				$('#branch').html(data);
				},
  		...
  		$('#branch').on('change', function() {
			...
			if(branch != '' && branch >0){
            	location.href = "{{ url('admin/inventory/stocks/fiberbranch-wise')}}?city="+city+"&distributor="+distributor+"&branch="+branch;
            }else{
				location.href = "{{ url('admin/inventory/stocks/fiberbranch-wise')}}";
			}
		});
        ...
        location.href = "{{ url('admin/inventory/stocks/fiberbranch-wise')}}?city="+city+"&distributor="+distributor+"&branch="+branch+"&category="+category;
      } else{
				location.href = "{{ url('admin/inventory/stocks/fiberbranch-wise')}}?city="+city+"&distributor="+distributor+"&branch="+branch;	
			}
		});
        ...
        location.href = "{{ url('admin/inventory/stocks/fiberbranch-wise')}}?city="+city+"&distributor="+distributor+"branch="+branch+"&category="+category+"&sub_category="+sub_category;
      } else {
				location.href = "{{ url('admin/inventory/stocks/fiberbranch-wise')}}?city="+city+"&distributor="+distributor+"&branch="+branch+"&category="+category;	
			}
		});
		...
		$("#brand").autocomplete({
			...
			select: function(event, ui) {
            	...
            	location.href = "{{ url('admin/inventory/stocks/fiberbranch-wise')}}?brand="+ui.item.value+"&category="+category+"&sub_category="+sub_category+"&city="+city+"&distributor="+distributor+"&branch="+branch;
			},
			messages: {
				noResults: '',
				results: function() {}
