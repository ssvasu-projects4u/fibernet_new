	...
	<div class="card-body" style="padding:5px;">
	  @if(Session::has('success'))
		...
		<?php
		   ...
		   // echo $querystring;
		     // $usertype = explode('-', $querystring);
		  //    echo $usertype[0];

		?>
		<div class="row">
		<div class="form-group col-md-3"> 
		{!! Form::label('sub_category', 'sub_category') !!}
		{!! Form::select('sub_category', $categories, $sub_category,array('required'=>'required','class' => 'form-control','placeholder'=>'-- Select Category --') ) !!} </div>
...
<!--<div class="table-responsive"> -->
	  ...
	  <tr class="table-warning">
		...
		<th>Product</th>
			<th>Length</th>
				<th>Starting_reading</th>
					<th>Ending_reading</th>
						<th>Drum_Number</th>
	...
	@foreach ($data as $datarow)
		  @php $user1 = DB::table('slj_stock_products')->select('id','product','length','dummy_startreading','dummy_endreading','drum_number')->where('id',$datarow->id)->first(); @endphp
	    @if(!empty($user1-> length) && !empty($user1-> dummy_startreading) && !empty($user1-> dummy_endreading )&& !empty($user1-> drum_number ))
	      <?php
		    $pr=DB::Table('slj_products')->where('id',$user1->product)->first();
		    ?>
        <tr>
        ...
        <ul class="dropdown-menu dropdown-default" role="menu">
          ...
          @can("transfer-from-stocks-to-employee")
      		  <li><a class="dropdown-item" href="{{url('admin/inventory/stocks/transfer-to-employee/manage-wise/'.$pr->id.'?'.$querystring)}}"><i class="fa fa-reply" aria-hidden="true"></i> Transfer to Employee</a></li>
          @endcan
        </ul>
        @endcanany
            </div>
		...
		<td>
			...
			<td>
			    <?php
			    if(!empty($user1->length))
			    {
			    ?>
			    {{ $user1->length }}
			    <?php } ?>
			    </td>
				<td>{{ $user1->dummy_startreading }}</td>
					<td>{{ $user1->dummy_endreading }}</td>
						<td>{{ $user1->drum_number }}</td>
  	...
  	<script type="text/javascript">
  		$('#category').on('change', function() {
			...
			if(category_id != '' && category_id >0){
            	location.href = "{{ url('admin/inventory/stocks/fibermanage')}}?category="+category_id;
            }else{
				location.href = "{{ url('admin/inventory/stocks/fibermanage')}}";
			}
		});
		...
		//	alert(sub_category);
			if(sub_category != '' && sub_category >0){
            	location.href = "{{ url('admin/inventory/stocks/fibermanage')}}?sub_category="+sub_category;
            }else{
				location.href = "{{ url('admin/inventory/stocks/fibermanage')}}?sub_category="+sub_category;	
			}
		});
		...
		$("#brand").autocomplete({
			...
			select: function(event, ui) {
            	...
            	location.href = "{{ url('admin/inventory/stocks/fibermanage')}}?brand="+ui.item.value+"&sub_category="+sub_category;
			},
			messages: {
				noResults: '',
				results: function() {}
