...
@section('content')
	...
	<div class="card-header py-2">
	  <div class="float-left"><h3 class="m-0 font-weight-bold text-primary">Transfer Stock to Employee</h3></div>
	...
	<div class="card-body">
	  @if(Session::has('success'))
        ...
        <?php } ?>
			...
			<div class="card shadow mb-4">
				<div class="card-header py-2">
					<div class="float-left"><h5 class="m-0 font-weight-bold text-primary">Transfer to Employee</h5></div>
				</div>
				<div class="card-body">	
				
				{!! Form::open(['name'=>'employee-submit','id'=>'employee-submit','route' =>'stocks.transfer-to-employee.submit','method'=>'post'])!!}
		  @csrf
		...
		<div class="form-group col-md-4"> {!! Form::label('employee', 'Employee') !!}

			<div class="input-group">
				<input type="text" id="employee_search" size="75" name="employee_search" class="form-control input-lg search-input ui-autocomplete-input" placeholder="Search employee" required autocomplete="off">				
			</div>
		...
		<input type="text" id="idtitle" size="75" name="search_string" class="form-control input-lg search-input ui-autocomplete-input" placeholder="Search for Autofill" required="" autocomplete="off">
       	  ...
       	  <div class="table-responsive">
				...
				@foreach ($data as $datarow)
					...
					<td>{{ $datarow->drum_number }}</td>
					<td>{{ $datarow->barcode }}</td>
					<td>{{ $datarow->start_reading }}</td>
					<td>{{ $datarow->end_reading }}</td>
	...
	<script src="{{asset('assets/js/autocomplete/jquery-ui.min.js')}}"></script>
  	<script type="text/javascript">
  		$(document).ready(function() {
			...
			$("#employee-submit").submit(function( event ) {
			...
			var src = "{{ route('search.employee.front.json') }}";
			$("#employee_search").autocomplete({
				...
				select: function(event, ui) {
        			...
        			$('#employee_search').append(ui.item.id);
