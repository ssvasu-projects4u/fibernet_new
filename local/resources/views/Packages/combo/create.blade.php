...
@section('content')
	...
	<div class="card-header py-2">
	  <div class="float-left"><h3 class="m-0 font-weight-bold text-primary">Create Combo Package</h3></div>
	...
	<div class="card-body" id="<?php echo rand(10000,1000000); ?>">
	  ...
	  {!! Form::open(['route' =>['combo-packages.store'],'id'=>'customer_submit','method'=>'post'])!!}
	      @csrf
		...
		<div class="row">
			...
			<div class="form-group col-md-4"> {!! Form::label('package', 'Broadband Package') !!}
								...
								<div class="vertical-menu" id="select3" style="height: 250px;overflow-y: scroll;">
									@if(isset($cabledatabytype['allacart']) && count($cabledatabytype['allacart'])>0)
										@foreach($cabledatabytype['allacart'] as $package)
											<div class="lab_allacart">
												<label class="radio-inline mr10"> <input type="checkbox" class="input_cable_allacart" value="{{$package['id']}}" name="cableallacart[]">&nbsp;{{$package['name']}} </label>
											</div>
										@endforeach
									@endif
								</div>
