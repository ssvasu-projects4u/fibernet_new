...
@section('content')
	...
	<div class="card-header py-2">
	  <div class="float-left"><h3 class="m-0 font-weight-bold text-primary">FH</h3></div>
	...
	@foreach ($data as $datarow)
        ...
        <button class="btn btn-primary btn-sm btn-demo-space py-0" data-toggle="dropdown"><i class="fas fa-bars"></i></button>
          ...
          <ul class="dropdown-menu dropdown-default" role="menu">
            @can("edit-fh")
              <li><a class="dropdown-item" href="{{url('admin/fh/edit/'.$datarow->id)}}"><i class="fa fa-pencil-alt" aria-hidden="true"></i> Edit</a></li>
            @endcan
            @can("delete-fh")
              <li class="divider"><hr class="py-0 my-0 mb-4"></li>
              <li><a class="dropdown-item" href="{{url('admin/fh/delete/'.$datarow->id)}}" title='delete' onclick="return confirm('Are you sure to delete?')"><i class="fa fa-trash" aria-hidden="true"></i> Delete</a> </li>
    ...
    <div class="modal-dialog" role="document">
		...
		<td>{{ $datarow->fiber_color }}</td>
		<td><a href="{{url('admin/fh/edit/'.$datarow->id)}}" title='edit'><i class="fa fa-pencil-alt" aria-hidden="true"></i></a> {{ $datarow->id	 }}</td>
		<td>{{ $datarow->dp_port }}</td>
		<td>{{ $datarow->generate_fh_id }}</td>
		<td>{{ $datarow->dp_num }}</td>
		<td>{{ $datarow->splitter }}</td>
		<td>{{ $datarow->splitter_core }}</td>
...
{{ $data->links() }}
  ...
  @php $data1 = DB::table('slj_fh')->where('user_id', $user_id)->get(); @endphp
